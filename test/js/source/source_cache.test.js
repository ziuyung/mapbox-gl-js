...
test('SourceCache#update', function(t) {
    t.test('loads no tiles if used is false', function(t) {
        ...
        sourceCache.on('load', function () {
            ...
            t.deepEqual(sourceCache.orderedIDs(), []);
            t.end();
        });
    });
    ...
    t.test('loads covering tiles', function(t) {
        ...
        sourceCache.on('load', function () {
            sourceCache.update(transform);
            t.deepEqual(sourceCache.orderedIDs(), [new TileCoord(0, 0, 0).id]);
            t.end();
        });
    });
    ...
    t.test('retains parent tiles for pending children (wrapped)', function(t) {
        ...
        sourceCache.on('load', function () {
            sourceCache.update(transform);
            t.deepEqual(sourceCache.orderedIDs(), [new TileCoord(0, 0, 0, 1).id]);
    ...
    t.test('includes partially covered tiles in rendered tiles', function(t) {
        ...
        sourceCache.on('load', function () {
            ...
            t.deepEqual(sourceCache.renderedIDs().length, 5);
            t.end();
        });
    });
    ...
    t.test('retains overscaled loaded children', function(t) {
        ...
        sourceCache.on('load', function () {
            ...
            sourceCache.update(transform);
            t.deepEqual(sourceCache.renderedIDs(), [
...
test('SourceCache#tilesIn', function (t) {
    ...
    t.test('reparsed overscaled tiles', function(t) {
        var sourceCache = createSourceCache({
            loadTile: function(tile, callback) { tile.loaded = true; callback(); },
            reparseOverscaled: true,
            minzoom: 1,
            maxzoom: 1,
            tileSize: 512
...
test('SourceCache#orderedIDs (ascending order by zoom level)', function(t) {
    var ids = [
        new TileCoord(0, 0, 0),
        new TileCoord(3, 0, 0),
        new TileCoord(1, 0, 0),
    ...
    var orderedIDs = sourceCache.orderedIDs();
