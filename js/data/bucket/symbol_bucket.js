...
var getBoxQuads = Quads.getBoxQuads;
    ...
    for (var p = 0; p < this.symbolInstances.length; p++) {
        ...
        if (hasText) {
            ...
            if (glyphScale <= maxScale) {
                ...
                this.addSymbols('box', symbolInstance.boxQuads, glyphScale, layout['text-keep-upright'], textAlongLine, collisionTile.angle);
            }
        }
...
SymbolBucket.prototype.addSymbols = function(programName, quads, scale, keepUpright, alongLine, placementAngle) {
    ...
    for (var k = 0; k < quads.length; k++) {
        ...
        var placementAngleLine = (Math.round(((angle%Math.PI)/Math.PI)*128)*2) + (symbol.alongLine ? 1 : 0);
...
function SymbolInstance(anchor, line, shapedText, shapedIcon, layout, addToBuffers, index, collisionBoxArray, featureIndex, sourceLayerIndex, bucketIndex,
    ...
    if (this.hasText) {
        ...
        this.boxQuads = addToBuffers ? getBoxQuads(anchor, shapedText, textBoxScale, line, layout, textAlongLine) : [];
